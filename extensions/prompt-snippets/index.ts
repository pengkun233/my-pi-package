 ...
 * placement, order). Bundled defaults live in the `snippets/` directory next
 * to this file; machine-local snippets live in `~/.pi/agent/snippets/` and
 * override bundled snippets with the same filename.
 *
 * - Press alt+shift+s or run /snippets to open the toggle menu (space: toggle,
...
const bundledSnippetsDir = join(extensionDir, "snippets");
export const customSnippetsDir = join(homedir(), ".pi", "agent", "snippets");
const snippetDirectories = [bundledSnippetsDir, customSnippetsDir] as const;
...
/**
 * Load bundled and machine-local snippets. Later directories override earlier
 * directories by filename, so local customizations win over packaged defaults.
 */
export function loadSnippets(directories: readonly string[] = snippetDirectories): Snippet[] {
	...
	for (const directory of directories) {
		...
		for (const file of readdirSync(directory).sort()) {
			...
			try {
				...
				if (snippet) snippetsById.set(snippet.id, snippet);
			} catch {
	...
	const snippets = [...snippetsById.values()];
...
export default function (pi: ExtensionAPI) {
	...
	async function openMenu(ctx: ExtensionContext) {
		...
		if (snippets.length === 0) {
			ctx.ui.notify(`No snippets found in ${snippetDirectories.join(" or ")}`, "warning");
			updateWidget(ctx);
			return;
		}
	...
	pi.on("session_start", (_event, ctx) => {
		enabled = new Set();
		snippets = loadSnippets();
		if (!existsSync(customSnippetsDir)) mkdirSync(customSnippetsDir, { recursive: true });
		updateWidget(ctx);
	});
