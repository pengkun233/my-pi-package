...
const allowed = /^(package\/(package\.json|README\.md|LICENSE|THIRD_PARTY_NOTICES\.md|install\.sh|config\/(packages\.json|global-agents\.md)|themes\/slop\.json|prompts\/(Get-Shit-Done|Neat-Freak|tidy-memory)\.md|skills\/wait-what-cn\/SKILL\.md|extensions\/(chat-mode\.ts|openai-usage\.ts|loop\/.*\.ts|memory\/.*\.ts|memory\/README\.md|ui\/.*\.ts)))$/;
